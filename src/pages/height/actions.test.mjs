...
test("readyText: 판정은 boolean 이다 — 번역된 문자열을 다시 검사하지 않는다", () => {
  ...
  assert.match(readyText(null).text, new RegExp(t("대기 중일 때만")));
});
...
test("jobSummary: 경과를 못 구하면 그 조각을 아예 안 붙인다", () => {
  ...
  assert.match(ok.text, /2분 0[0-9]초 경과/);
  ...
  assert.doesNotMatch(bad.text, /경과/);
  assert.match(bad.text, /격자/);
...
test("오류 문구: 재시도해도 소용없는 쪽을 그렇게 말한다", () => {
  ...
  assert.match(startErrorText({ body: { code: "no_optics" } }), new RegExp(t("캘리브레이션을 먼저 발행하세요")));
...
test("manualHeightError: cm 를 넣는 실수를 왕복 전에 잡는다", () => {
  ...
  assert.match(manualHeightError("650").error, /1~30/);
  assert.match(manualHeightError("0.5").error, /1~30/);

...
import { SetupPanel } from "./setup-panel.jsx";
import {
...
export default function SimulatorPage() {
  ...
  // ── 슬롯 선택 ─────────────────────────────────────────────────────────────
  // 평면도와 오른쪽 목록은 **같은 슬롯을 두 방식으로 그린 것**이라 고르는 자리도 하나다.
  // 그래서 선택은 여기가 들고, 그 차의 값을 읽어 폼에 올리는 일은 폼을 가진 쪽이 한다.
  ...
  // 탭을 열 때만 하는 일 — 리그는 이 화면 밖에서도 바뀐다.
  // (설정 탭의 주소와 씬 탭의 저장 목록은 각 패널이 스스로 읽는다. 그 패널들은 자기 탭에서만
  //  마운트되므로 마운트가 곧 「탭을 열었다」이고, 여는 쪽과 읽는 쪽을 갈라 둘 이유가 없다.)
  useEffect(() => {
    if (!booted) return;
    if (tab === "rig" || tab === "drive") refreshRig();
  }, [tab, booted, refreshRig]);

  // 저장본을 복원하면 카메라도 차량도 바뀐다 — 씬·기기 목록·슬롯을 전부 다시 읽는다.
  // 복원 자체는 씬 탭 패널의 일이고, 그 뒤를 수습하는 이 일은 화면 전체를 아는 여기의 몫이다.
  const reloadAfterRestore = useCallback(async () => {
    catalogRef.current = null;
    await refreshRig();
    await loadScene();
  }, [refreshRig, loadScene]);
    ...
    <main id="sim-main">
      ...
      <div id="sim-left">
        ...
        <div id="sim-bottom-tabs" className={tab === "rig" || tab === "drive" ? "tall" : ""}>
          ...
          {tab === "info" && (
            <ScenePanel locked={locked} catalog={catalog} cameras={cameras}
                        setBusy={setBusy} log={log} onRefresh={loadScene} onReset={simReset}

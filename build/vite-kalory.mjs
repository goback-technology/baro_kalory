...
//
// dist 의 URL 공간: **index.html 하나가 전부다.** 옛 주소(`<slug>.html`·`/v0.html`)에는
// 리다이렉트 셸이 발행된다. 그 밖에 .nojekyll·app-versions.json·favicon·test/.
...
// 옛 주소 → 셸의 해시. dev 에서는 302 가, dist 에서는 리다이렉트 셸이 같은 일을 한다.
...
export function kaloryCompat() {
  return {
    ...
    // "./web/…" (셸의 모듈 참조) → src/…. dev 와 build 가 같은 훅을 탄다 — dev 서버는
    ...
    // dev 전용: 옛 주소(확장자 있든 없든)와 뒤끝 슬래시 별칭을 셸의 해시로 보낸다.
    ...
    closeBundle() {
      ...
      // 옛 주소 — 북마크가 404 가 되지 않게 셸을 발행한다.
      for (const p of PAGES) {
        writeFileSync(join(dist, pageFileOf(p)), redirectShell(hashOf(p), p.label));
      }
      writeFileSync(join(dist, "v0.html"), redirectShell(hashOf(CCTV), CCTV.label));

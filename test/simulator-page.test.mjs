...
test("카메라를 바꾸면 그 시뮬레이터의 씬을 다시 읽는다", async () => {
  ...
  assert.match(src, /if \(!catalogRef\.current \|\| !S\.current\.catalog\) loadScene\(\);/,
    "빈 카탈로그는 폴링이 스스로 회복해야 한다 — ref 와 화면 상태 **양쪽**을 봐야 한다"
    + " (ref 만 보면 invalidateScene 뒤 캐시 따뜻+화면 공백을 영영 못 알아챈다, 2026-08-27 실측)");
  // 화면 상태는 캐시 히트든 미스든 loadScene 이 항상 세운다 — setCatalog 이 if 안에만 있으면
  // 상태만 비우고 캐시를 남기는 invalidateScene(카메라 전환) 뒤에 화면이 영구 공백이 된다.
  assert.match(src, /if \(cat\?\.cars\?\.length\) catalogRef\.current = cat;\s*\n\s*\}[\s\S]{0,400}?setCatalog\(cat\);/,
    "setCatalog 은 캐시 분기 **밖**에서 항상 불려야 한다");
